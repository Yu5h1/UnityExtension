...
- Three independent performance axes on `SolverOscillationProfile`, all 0..1, with `drive = vitality * (1 - stiffness)` scaling the pose correction. `plan.md` section 15.3 is the reference.
  - `stiffness` is hardness and works in two opposing directions at once, which is what separates it from vitality: it converges particle velocities onto the instance mean, which removes relative motion and freezes whatever shape the body currently holds, and it lowers the drive. It carries no target shape of its own. An earlier version had it pull toward a straight pose, so full hardness straightened whatever it froze.
  - `vitality` is willingness to move. Both it at 0 and stiffness at 1 give `drive = 0`, but the first leaves the body limp for contact to reshape and the second locks the shape, which is the boundary between the two axes.
  - `muscleTension` replaces `bendRatio` and chooses the target shape through `asin(1 - muscleTension)`. 1 targets the topology's own rest form, which is the muscle-cramp behaviour. Note 0 is the geometric limit at 90 degrees rather than a natural amplitude; useful range is about 0.2 to 0.4.
- `SolverParticleProfile.torsionAlign` rotates each control group about the body axis to bring the segments' cross directions back into agreement. Nothing else can: every driver moves a whole group by one shared delta so the cross vector never changes, roll damping only removes the rate of twist, and the topologies are planar at rest so their diagonals resist twist only to second order. The mesh takes its frame from each segment separately, so a twisted body skins into a contorted shape even when its controls are collinear.
...
- Apparent bend speed is `peakHalfAngle * angularFrequency`, so amplitude and rate both read as speed and cannot be separated in a continuous wave. `muscleTension` at 0 gives 90 degrees, roughly double the amplitude the retired `bendRatio` default produced, which reads as frantic. A `burstDuration` split was tried and reverted for not addressing this; see `plan.md` section 15.7.
- `muscleTension` at 0 maps to the geometric limit rather than a natural amplitude, which contradicts reading 0 as the relaxed resting state. Unresolved: either treat 0.2 to 0.4 as the working range, or remap and expose the limit separately.
