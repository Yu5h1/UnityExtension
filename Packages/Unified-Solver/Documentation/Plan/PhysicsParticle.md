...
已經做完，行為與參數記在 `plan.md` §18.2。這一節保留**為什麼這樣分**，因為那個推理在下次有人想把係數搬回介質上時才有用。
...
同一個 `ρ`。所以介質只需要 `density` 與 `flow`。

**錯位在於係數的歸屬。** `C_d`、面積、質量都是**身體**的性質，不是介質的。今天 `viscosity` 掛在介質上，等於把「你有多容易被拖動」寫成了水的性質。
...
阻力率 = `density × 身體的阻力係數`。推進力同理。

**不是模式旗標，是兩個係數。** 自體推進永遠是「推某個東西」，差別只在推流體還是推接觸面。但那不該是 locomotion 上的二選一，**因為鴨子兩種都會**：

| 係數 | 魚 | 鳥 | 羊 | 鴨 |
|---|---|---|---|---|
| 介質推進 | 高 | 低（靠升力，不靠拍打） | 0 | 中 |
| 接觸推進 | 0 | 低 | 高 | 中 |

係數為 0 就自然表達「這個身體不會這樣移動」，不需要額外的 enum，也不會逼作者在鴨子身上二選一。

**在海底走路很吃力會自己發生**，不需要為它加參數：浮力減少接觸力 → 摩擦是 `mu × penetration` → 抓地力自動變小；同時阻力仍在。兩個機制都已經在系統裡，只等接觸推進被實作。

**一個常見的誤解要澄清**：密度再高也只會讓東西浮在表面，不會擋住它進入。擋住是碰撞體的工作，不是介質的。

**代價**（已付）：這條路把一個參數從介質搬到身體，**現有每一個介質的調校都會變** —— 水、水管、冰塊的手感都要重調一次。換到的是參數之間不再互相矛盾，而且新介質（空氣、糖漿、泥）只要填一個真實密度就有合理行為。

一個具體的後果值得先寫下來：水管不再需要 `density = 0` 的假水。**水管就是水加上高流速**，身體拿到全額推進力但輸給流速 —— 和 §18.2 那個速度比較是同一件事，只是不再需要任何不物理的設定去達成。
