...
### 13.13 介質體積（Medium Volume）

`SolverMediumVolume` 是一段**替換全域環境的區域**：離開它就回到 solver 的全域重力與無阻力狀態，進入它就由該介質接管。與碰撞體的關係相同 —— 都是區域，差別在碰撞體阻擋空間，介質改變空間由什麼構成。

形狀取自 Transform（position 為中心、lossyScale 為完整尺寸、rotation 定向），註冊是全域靜態清單而非逐 emitter，因為介質屬於場景而不屬於在裡面游的東西。

**預設是 Box，理由是水面。** 橢球沒有平頂 —— 它的表面高度隨水平位置變化，所以浮起來的物體會依所在位置停在不同高度，那不是水面。Box 的上表面就是一條真正的水線。橢球保留給「沒有水面可言」的介質：大水體內部的一道水流或渦流。

浮在水面不需要額外機制：浮力是逐粒子的，跨越頂面的物體有些粒子被抬有些沒有，自然停在部分沒入的平衡點。
...
| | 式子 | 行為 |
|---|---|---|
| **density** | `a = g × (1 − medium / body)` | 比值，不是覆寫 |
| **flow / viscosity** | `v += (flow − v) × (1 − e^(−viscosity·dt))` | 收斂到目標速度 |

**密度是比值，所以同一片水裡重的沉、輕的浮。** 若改成「在體積內覆寫重力向量」，重冰塊和輕冰塊會一起浮或一起沉 —— 那不是密度的意思。物體密度由粒子自身質量與 solver 全域 `particleRadius` 算出。

單位由 profile 質量與粒子半徑決定，**不是 kg/m³**，所以水不是 1000。調到東西停止下沉的那個值就是中性。

**流動是速度目標，不是加速度。** 若寫成加速度，最終速度會由全域 damping 決定 —— 設 5 得到 2.5 m/s 只因為 damping 剛好是 2，之後調 damping 會無聲地改掉場景裡每一道水流。指數形式而非 `viscosity × dt`，後者在乘積超過 1 時會衝過頭並震盪。

涵蓋範圍：

| 情境 | density | flow | viscosity |
|---|---|---|---|
| 水 | = 物體密度 | 0 | 中 |
| 死海 | > 物體密度 | 0 | 中 |
| 洋流 | = 物體密度 | (1,0,0) | 中 |
...
吃水線因此免費得到：魚跨越水面時部分粒子在內、部分在外，浮力自然只作用在水中那些，魚就浮在水面 —— 不需要任何東西去模擬「水面」。

#### 全域 damping 保留

它是 solver 的能量洩放機制，不是黏滯的替代品。PBD 透過約束修正注入能量（那個 1/subDt 放大），沒有全域衰減的話能量無處可去。黏滯是**額外疊加的局部阻力**。

#### 明確不做：游泳

擺動驅動是**動量中性**的（每個 delta 都減掉加權平均），所以彎曲身體本身不產生任何淨位移 —— 水裡水外都一樣。而**線性等向黏滯也改變不了這件事**：往復的彎曲循環在其中正反兩半衝量精確抵消（Scallop Theorem）。

推進力需要**各向異性阻力**（橫向阻力大於縱向），這正是真實細長體推進的原理，也是下一步。

同一個事實也解釋了為什麼「離水失去自主力」不需要任何程式碼 —— 它已經成立了。
...
### 15.12 尾偏（tailBias）：節奏不再對稱

`SolverOscillationProfile.tailBias`，0 到 1，預設 0.5。

回答的是「動作忽略了頭重尾輕」。原本驅動讓頭段與尾段繞中段旋轉**相同的** `halfAngle`，兩端掃過一樣的弧。`tailBias` 把這個總量在兩段之間分配：

```text
headPeak = 2 × peakHalfAngle × (1 − bias)
tailPeak = 2 × peakHalfAngle × bias
```

0.5 精確重現原本的對稱姿勢，所以既有 asset 不受影響，這個欄位可以不理它。

這是先前記過「需要非對稱驅動才能偏航」的那個機制，不只是外觀：**對稱的 C 形在任何振幅下都無法轉動頭尾弦的方向**。

兩半各自受 `MAXIMUM_HALF_ANGLE` 上限，**分開夾**。因為分配保留總量，全偏時等於要求單一段做兩倍的對稱角度，只夾總和的話驅動會重新碰到髮夾。

偏移不會讓髮夾更糟：單段 70°、另一段 0° 時頭尾弦是身長的 82%，而兩段平均彎曲時只有 34%。

**質量仍然是均勻的。** 每顆粒子帶 `profile.mass / particleCount`，動量平衡對頭尾一視同仁，所以這是**驅動**的偏移而不是**身體**的偏移。真正的頭重尾輕是另一件事，且與下面 22 節的質量語意問題共用同一個阻塞點。

## 16. 體積是幾何，效果是那裡發生什麼

`SolverVolume`（場景元件：形狀加一串效果）與 `SolverVolumeEffectProfile`（抽象 ScriptableObject）。`SolverMediumProfile` 是第一個效果，13.13 描述的就是它。

### 16.1 為什麼拆

**幾何是複製起來昂貴的那一半，而且不管什麼效果它都一模一樣。** 若採一個行為一個元件，範圍限制器就會帶著第二份形狀程式碼、第二份註冊清單、第二個 GPU struct 與第二次 inside test 到場 —— 而一座水體與邊界是同一個箱子的水族箱，會需要兩個 Transform 靠人工保持同步。

- **一個 (volume, effect) 配對一筆，攤平放進同一個 buffer。** 一個箱子掛兩個效果就上傳兩筆相同幾何，代價是 inside test 每個效果重做一次。在場景的體積數量級下，這比另一個選項（GPU struct 內放變長清單）便宜。
- **Payload 是無型別的。** `SolverVolumeGPU` 帶 `payloadVector` 加 `payloadX/Y/Z`，只有效果的 `Write` 與對應的 kernel 分支知道映射。這正是「一個 buffer、一次上傳、一次幾何測試服務所有效果」的代價與收益。介質原本的三個 padding float 剛好就是效果標籤、側邊旗標與額外純量需要的空間，所以 stride 仍是 96 bytes。
- **`actOutside` 屬於效果，不屬於體積。** 催生整個設計的案例就是一個箱子同時作用於兩側 —— 內部是水、外部回收 —— 那是體積層級的旗標表達不出來的。
- **`Granularity` 是宣告出來的，而且不是效能註記。** 介質是逐粒子，那是「沒有任何東西模擬水面，物體卻停在吃水線」的原因。回收判定是逐實體，因為半條魚無法被回收。宣告錯的效果不是稍微錯，是在回答另一個問題。只存在於 C# 側；kernel 本身**就是**那個迴圈，所以它已經知道自己的粒度。
- runner 的 `UploadVolumes()` 不知道任何效果的存在。新增一個效果是一個 enum 值、一個子類別、一段 kernel 分支，不碰體積也不碰上傳路徑。
- 刻意鏡射 `SolverParticleProfile.modifiers`：「一串被編寫的行為」只有一種模式要學，不是兩種。

### 16.2 遷移

- `SolverMediumVolume` → `SolverVolume`。`.cs` **與它的 `.meta`** 用 `git mv` 一起搬，GUID 存活，既有場景元件照樣解析。類別上另有 `[MovedFrom]` 處理型別名稱。
- `SolverMediumProfile` 保留名稱與 asset GUID，只是多一個基底類別，所以沒有 `.asset` 需要重新編寫。
- 舊的單一 `profile` 欄位由隱藏的 `[FormerlySerializedAs("profile")] _legacyProfile` 接住，存取器把它當成已經在清單裡呈現，`OnValidate` 在編輯器首次檢視時折進 `effects`。保留為獨立欄位而非載入時遷移，這樣開啟舊場景但不存檔不會弄丟它。
- `SolverMediumShape` → `SolverVolumeShape`。`shape` 欄位保留名稱與數值，int 存活；若 enum 型別變更讓 importer 混淆，損失僅是重選一次 Box。

### 16.3 介質的喚醒通道

其他每一個喚醒訊號都是**測量**，之所以可行是因為其他東西都透過 solver 抵達沉睡的實體。介質不是：它的兩個通道是速度與（透過速度的）位置，而那恰好是 `ApplySleep` 覆寫掉的兩個。沉睡的實體在水流中不是反應慢，是**結構上無法察覺**，而位移測試接著忠實地回報一個從未被允許移動的實體。

所以改成直接問介質。`ApplyMedium` 現在逐實體寫出，與沒入度並列：

- **drift**，實體上 `length(mean(flow − velocity))`，乾的粒子貢獻零。它已經是速度、也已經加權過沒入度，所以能直接與 `sleepSpeed` 同單位比較。
- **lift**，沿 up 的淨環境加速度，**含重力**。沉底的重碎片讀到負值而睡著；有浮力的讀到正值而上升；浮在水線上的大約讀到零，因為乾的那半把它拉回去。用 `sleepDelay` 換算成速度。

測試放在狀態分支之前，所以它同時喚醒沉睡者與阻止候選者入睡 —— 同一個條件的兩面。

**刻意的後果：** 在流速高於 `sleepSpeed` 的水流裡沒有東西睡得著，不論摩擦是否真的按得住它。被推是清醒的理由，接觸仍然決定實體會不會移動，但這表示流動的水裡 sleep 實質上是關閉的。水流只是佈景時，把 `sleepSpeed` 調到流速之上。

`_Submersion`（單一 float）因此改為 `_MediumState`（float4：沒入度、drift、lift、保留）。

### 16.4 一併修掉的

- `submerged` 原本是每個 (粒子, 體積) 配對各加一次，兩個重疊體積會回報大於 1 的沒入度，而 locomotion 拿它去乘推力。現在每顆粒子只算一次。
- `ApplyMedium` 原本在 `_MediumCount == 0` 時**早於**寫入沒入度就返回，於是離開水的實體保留上一步的讀數並繼續游泳。現在清除狀態再返回。

### 16.5 尚未解決

- **浮力可能抬不起貼地的實體。** 接觸在同一個 substep 內由位置重建速度，所以真正碰到地面的那些粒子的寫入會被覆蓋。未測試；這是本設計最可能令人失望的地方。新的 lift 項至少能喚醒這樣的實體，那是它先前做不到的。
- 每個 emitter 的 runner 各自上傳整份介質清單。正確，但在多個 emitter 時重複。

## 17. Bounds：離開、淡出、回來

`SolverBoundsProfile`，第二個效果。

- **觸發與包絡是分開的。** 觸發是 bounds 效果落在某個實體上；包絡是掩護傳送的淡出淡入。兩個時長都設 0 是合法設定，而 `actOutside` 關掉就把同一個效果從邊界變成殺傷區。這一對不需要額外成本，是組合式設計在第一次使用就回本。
- **傳送要與 `position` 一起寫 `prevPosition`，這就是全部關鍵。** Solver 以 `(position − prevPosition) / subDt` 重建速度，所以只寫位置的跳躍會回傳成「整段距離除以一個 substep」—— 跨越場景就是數百 m/s，抵達出生點的瞬間已經在離開。速度歸零是同一個道理的鏡像：被甩得夠用力而越界的實體，會立刻再越界一次。
- **淡出用縮小，不用混色。** Alpha 意味著透明佇列，隨之而來的是排序、失去 depth prepass 與另一條陰影路徑 —— 一個玩法功能夾帶進來的渲染決策；而在剛體路徑上它會吐回「任何 URP material 都能用」這個刪掉 `SolverRigidMesh.shader` 才贏得的性質。把身體收縮到中段 frame 的中心維持不透明、不需要改材質，而且歸零時三角形退化所以什麼都不畫。樞紐**必須**是中段 frame 的中心；收縮到各自 frame 的中心只會把身體變細而不變短，最後留下一條線。
- **生命週期狀態由 emitter 擁有**，因為 runner 寫它、renderer 讀它，兩者互不擁有 —— 與它們已經共用 `InstanceBuffer` 的關係完全相同。`float4(state, phase, hidden, rate)`，而儲存 *hidden* 而非 visible，正是讓一個從未被觸碰的全零 buffer 就代表「活著且完全繪製」的原因。沒有 bounds 效果的場景永遠不寫它。
- **回收中的實體被逐一強制喚醒。** Sleep 會還原記錄下來的姿勢，而那個姿勢早於傳送，所以它每幀都會把實體拖回界外並且贏。從生命週期 buffer 讀取而非透過全域 `_KeepAwake`，因為只有部分實體在週期中。
- **無條件 dispatch。** 以「邊界存在」為條件會讓最後一個邊界被移除時仍在淡出中的實體擱淺：看不見、永遠、而且沒有任何還在跑的東西能替它收尾。Kernel 讀一次 buffer 就退出。
- 決定只做一次：淡出途中又飄回界內的實體仍然回家。半途取消需要第四個狀態而且沒有好處，因為那時實體已經半消失了。
- 重生是實體的**剛性平移**到 emitter spawn box 內的新點，不是從拓樸 rest pose 重建。形狀與朝向延續，而淡入淡出的存在就是為了讓這件事不引人注意；同時 spawn 佈局留在一個地方，不必在 GPU 上複製一份。

已知缺口，兩個都是真的：

- **剛體路徑沒有淡出。** `Graphics.RenderMeshInstanced` 吃 CPU 端建好的矩陣，而生命週期活在 GPU 上，所以碎片會正確回收但是用瞬間跳的而不是縮小。要補需要 `AsyncGPUReadback` 鏡像（對視覺包絡而言延遲無害），或把剛體實體的 bounds 測試移到 CPU 側。
- **淡出中的實體仍以完整粒子半徑碰撞。** Solver 完全不知道 renderer 在縮放任何東西，所以長時間淡出讀起來像一個還會推人的鬼魂。把時長保持得短。
- 若 emitter 的 spawn box 落在邊界之外，實體會永遠回收下去。目前沒有東西檢查這件事。

## 18. Locomotion 與轉向

`SolverMotionTarget`（場景：Point 或 Direction，加一個到達半徑）與 `SolverLocomotionProfile`（modifier：速度、頻率、時長、隨機度）。

- **命名為 locomotion 而非 swimming。** 魚、蛇與獸群的差別在於推什麼、看起來像什麼，不在速度與節奏。
- **在這裡加入動量是正當的，不是抄捷徑。** 動物靠把某個東西往後推來移動；介質就是被推的那個，而反作用力屬於沒有被模擬的水。這也是它以沒入度為閘門的原因：離開介質就沒有東西可推，所以離水的魚會自然癱軟，不需要一條規則說它該癱。
- **滑行是湧現的。** 一次推進把平均速度推向 `direction × speed`；推進之間**什麼都不寫**，由介質的黏滯把速度洩掉。真實魚類的加速—滑行—加速節奏就是這兩個機制的合成，不是第三個狀態。
- 持續移動不需要模式：`duration >= 1 / frequency` 就沒有留下滑行的空隙。與 `SolverOscillationProfile` 同形。
- **速度是被編寫的，不是力。** 力會讓介質的黏滯決定場景裡每隻動物能跑多快 —— 重調水就重調了野生動物。
- 作用於實體平均，所以身體自身的彎曲不受影響。
- 目標帶到達半徑，所以多個群組不需要 id：實體跟隨最近且搆得到它的目標，半徑 0 搆到所有實體。
- 內插、Spline 與 Timeline 刻意缺席。它們是從外部驅動目標的 Transform，Unity 已經做得比這裡寫的任何東西都好；餵餌就是移動那個物件。

### 18.1 轉向

平移一運作就暴露出來的缺口：實體正確地移動，然後保持躺平，因為沒有任何東西旋轉它們。

- `turnRate` 把身體自身的切線盪向航向，`uprightRate` 把它的法線滾向世界上方。**兩個獨立的軸**：實體可以精確地指向它前進的方向，同時仍然側躺著，因為沒有別的東西抵抗繞長軸的旋轉。`uprightRate` 為 0 適合沒有上方可言的動物。
- 以**繞身體中心的角速度**施加，絕不以旋轉位置的方式。旋轉位置正是 `torsionAlign` 做過的事，它會穿透身體倚靠的東西並讀起來像在跳。速度無法穿透任何東西，而 `cross(omega, r)` 在對稱的偏移上是動量中性的。
...
- 硬上限讓所有超過門檻的東西在同一幀以同一個速度出來。那讀起來很假，而且摧毀真實資訊：實體被撞得多重。衰減超出量保留了排序，只壓縮了分布。
- 門檻以下完全不作用，這是它能用在全域 damping 用不了的地方的原因。全域 damping 曾為了壓制飛射被調到 2，結果讓一般運動變得遲鈍；有了這個它可以調回去。
- 作用於實體平均速度，所以相對運動不受影響，尾巴仍然跑得比自己的中心快。對每顆粒子加同一個修正量會縮放平均而讓每一個差值原封不動。
- 只搆得到自由的實體。凡是接觸或 shape matching 在同一個 substep 內寫入位置的地方，速度都會由位置重建，所以這不是摩擦，也不是 Sleep 的替代品。這個範圍是刻意的：它的存在是為了被穿透修正或被拖動的碰撞體甩飛的實體。
- `maxDepenetrationSpeed` 是不同種類的上限而且維持硬性：它在 substep 迴圈內限制位置修正量，正是為了讓由此產生的速度不會爆炸，而軟膝會放大值通過。它也只作用於粒子對粒子的接觸，從不作用於世界碰撞體，所以它根本不管被拖動的碰撞體。

## 20. 噴流就是介質

「水管噴水，東西看起來像被沖走」由既有的箱形 `SolverVolume` 掛一個 `density = 0` 的 `SolverMediumProfile` 回答 —— 純推力、不加浮力。13.13 的前提本來就涵蓋它：水、空氣、沙與水流是同樣三個數字，噴流是 `flow` 很大的那一個。

- **ParticleSystem 畫水，而且只畫水。** 兩者共用一個 Transform、不共用任何 state，這就是《ParticleSystem × Unified Solver》第 3 節的單一權威規則，而且沒有剩下任何需要同步的東西。
- **PS 粒子不能也不該對 solver 粒子傳訊息。** PS 粒子資料在 CPU 側而 solver 粒子是 GPU buffer，所以那會是該文件第 9 節列為非目標的每幀雙向同步，外加一次為了知道哪顆水滴打到什麼而做的第二次 broad phase —— vendored 的 spatial hash 並未對外開放。而且結果甚至不對：一顆水滴是一次微小衝量，合起來是抖動。讀起來像**被沖走**的是持續的方向性速度場，而介質本來就是那個。
- **`flowIsLocal` 存在是因為 profile 是共用 asset。** 世界座標的 `flow` 在每個引用它的體積裡都是同一個向量，所以旋轉噴嘴不會改變噴射方向，而兩根朝不同方向的噴嘴無法共用一個 profile。在 `Write` 內解析，因為問題是「被編寫的數字代表什麼意思」，那是效果的職責；kernel 兩種情況下都收到世界座標的 flow，永遠不知道曾經有過這個決定。預設關閉，既有 asset 不受影響。洋流正是要它關閉的案例：它是世界的性質，自己沒有朝向。
- 一個體積免費覆蓋所有 emitter —— `SolverVolume.Registered` 是靜態的，每個 runner 都上傳整份清單 —— 所以冰塊與魚不需要接線，也不需要第二份 asset。
- Box 換來三件事，暫時接受：體積內部推力均勻、邊界是硬切、沒有擴散。若實機讀起來不對，逃生口是新增 `SolverJetProfile`，方向取自體積已經上傳的 `axisZ`（不佔 payload），速度／收斂率／衰減指數放 payload，分支寫進既有的 `ApplyMedium` 迴圈，沿用同一份幾何、同一次上傳與同一條 drift 喚醒通道，並刻意**不**計入沒入度。
- 兩個既有互動要先查再怪機制：`speedLimit` 低於噴射速度會衰減推力而讀起來像水管沒力；任何介質都會設定 `submerged` 而啟動 `ApplyLocomotion`，所以在**空氣中**被噴到的魚會開始游泳（在水族箱裡它本來就沒入，這條不成立）。
- PS 的 `Collision` 只看得見 Unity 碰撞體，所以水花不會從 solver 實體上濺開。在目標上加一顆純視覺用的 Unity 碰撞體是唯一的辦法，而 solver 看不見它。

## 21. Sleep 實作：13.8 之後學到的事

13.8 是設計意圖，這一節是實際建出來之後才知道的。

- **`settle` 從來就不可能成立。** `UnifiedSolver.compute` 在**每個 substep** 結尾都做 `p.velocity = (p.position − p.prevPosition) / subDt`，直接覆寫速度。Modifier 每個 FixedUpdate 跑一次、在那個迴圈之外，所以它寫的速度只活過 `substeps` 個 Predict 中的一個 —— 30 個 substep 大約 3%。`settleSpeed` 因此被移除，不是調校問題而是結構問題。
- **`ApplySleep` 寫的是位置。** 設定 `prevPosition = position` 才是讓「停住」真的停住的原因，因為那個差值是 `UpdateVelocity` 唯一讀的東西。
- **喚醒依據位移，不是速度。** 沉睡實體的速度被 kernel 按在零，所以它無法回報自己的運動；solver 設法把它推了多遠才是誠實的訊號，而且一次涵蓋所有來源。
- **執行順序是機制的一部分。** runner 在 `[DefaultExecutionOrder(50)]`，而 `SolverManager` 沒有宣告所以在 0。我們的 kernel 因此在 solver 步進**之後**才跑，那正是「先觀察再修正」得以成立的原因。任何重排都會破壞 sleep。
- Buffer 依 `maxInstances` 配置，所以一個 slot 永遠對應一個實體索引，並且明確清零：未定義的 ComputeBuffer 內容會被讀成「實體已經睡著，姿勢是記憶體裡剛好那些東西」。
- 僅限擴充層。它停下可見的運動，但仍然跑完每一個 solver kernel；要略過它們需要打開 vendored 相依，那是另一次授權。

### 21.1 Sleep 會把實體卡死，以及它如何說出來

在 Unity 中確認：`sleepSpeed = 0` 時碎片從不卡在牆裡，所以 Sleep 就是元凶。**維持姿勢這件事本身就是讓被卡住的實體無法回報的原因**：還原每幀撤銷穿透修正，位移永遠長不大，喚醒測試永遠不觸發。它按得越用力，實體越沒有能力抱怨。

- **持續性區分了大小區分不了的兩種情況。** 真正靜止的實體沒有東西在推它，所以修正量基本上是零；被卡住的每幀回傳同樣的修正量。持續高於 `wakeDistance` 十分之一、且持續達 `sleepDelay` 的位移現在會喚醒它。不需要新欄位：兩個數字都由既有控制項導出。
- **喚醒是重試，不是修復。** 還原停止，solver 得到一次乾淨的機會把實體推出去，然後它在落腳處重新睡著。仍然卡住就重複這個循環，那是正確的行為。
- 沉睡分支重用 `state.y` 當持續推擠計時器，因為它清醒時持有的候選倒數與此不重疊。
