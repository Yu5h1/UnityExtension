...
namespace Yu5h1Lib.EditorExtension
{
    ...
    public class ThemeEditor : Editor<Theme>
    {
        ...
        #region MenuItem
        private const string MENU_PATH = "CONTEXT/Theme/";
        private const string ShowRelatedSubAssetsMenu = MENU_PATH + "Show Related SubAssets";

        private static Object[] GetRelatedSubAssets(Theme theme, out HashSet<Object> subAssetSet)
        {
            var path = AssetDatabase.GetAssetPath(theme);
            var allAssetsAtPath = AssetDatabase.LoadAllAssetsAtPath(path);
            var founds = new HashSet<Object>(
                allAssetsAtPath.Where(a => a != null && a != theme)
            );
            subAssetSet = founds;

            var itemsArray = theme.items?.Where(i => i != null).Cast<Object>().ToArray();
            if (itemsArray == null || itemsArray.Length == 0) return null;

            var itemSet = new HashSet<Object>(itemsArray);

            return EditorUtility.CollectDependencies(itemsArray)
                .Where(d => founds.Contains(d) && !itemSet.Contains(d))
                .ToArray();
        }

        [MenuItem(ShowRelatedSubAssetsMenu, true)]
        private static bool ToggleShowSubAssets_Validate(MenuCommand command)
        {
            var theme = command.context as Theme;
            if (theme == null) return false;

            var relatedSubAssets = GetRelatedSubAssets(theme, out _);
            if (relatedSubAssets == null || relatedSubAssets.Length == 0) return false;

            bool isVisible = (relatedSubAssets[0].hideFlags & HideFlags.HideInHierarchy) == 0;
            Menu.SetChecked(ShowRelatedSubAssetsMenu, isVisible);
            return true;
        }

        [MenuItem(ShowRelatedSubAssetsMenu, false)]
        private static void ToggleShowSubAssets(MenuCommand command)
        {
            var theme = command.context as Theme;
            if (theme == null) return;

            var relatedSubAssets = GetRelatedSubAssets(theme, out _);
            if (relatedSubAssets == null || relatedSubAssets.Length == 0) return;

            bool allVisible = relatedSubAssets.All(a => (a.hideFlags & HideFlags.HideInHierarchy) == 0);

            foreach (var asset in relatedSubAssets)
            {
                if (allVisible)
